...
#     cluster_features = df.drop(columns=["trigger_recommended"])
...
def preprocess_data():
    ...
    # -----------------------------
    # 2. Patient-level aggregation (KEY FIX)
    # -----------------------------
    patient_features = (
        df
        .groupby("patient_id")
    ...
    # -----------------------------
    # 3. Patient-level clustering
    # -----------------------------
    scaler = StandardScaler()
    patient_scaled = scaler.fit_transform(
        patient_features.drop(columns=["patient_id"])
    )

    kmeans = KMeans(n_clusters=3, random_state=42)
    patient_features["cluster_id"] = kmeans.fit_predict(patient_scaled)

    # -----------------------------
    # 4. Merge cluster_id back to visits
    # -----------------------------
    df = df.merge(
        patient_features[["patient_id", "cluster_id"]],
        on="patient_id",
        how="left"
    )
    ...
    # -----------------------------
    df.to_csv("data/processed/trigger_day_processed.csv", index=False)
    print(" Data preprocessing + patient clustering completed")
